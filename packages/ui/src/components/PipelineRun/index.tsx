...
export function PipelineRun({ commit, message, branch, duration, ago, status, steps, affectedApp }: PipelineRunProps) {
  ...
  return (
    <div style={{ border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '14px', background: 'var(--color-surface)', borderBottom: '0.5px solid var(--color-border)' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--color-text-3)', letterSpacing: '1px', marginBottom: '4px' }}>
            {ago} · {branch}{affectedApp ? ` · ${affectedApp} only` : ''}
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--color-text)', marginBottom: '2px' }}>{message}</div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--color-text-3)' }}>{commit}</div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: '16px' }}>
          <span style={{ display: 'inline-block', fontFamily: 'var(--font-mono)', fontSize: '8px', letterSpacing: '1px', padding: '3px 8px', borderRadius: 'var(--radius-sm)', background: s.bg, border: `0.5px solid ${s.border}`, color: s.color }}>{s.label}</span>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--color-text-3)', marginTop: '5px' }}>{duration}</div>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap', padding: '10px 14px', background: 'var(--color-elevated)' }}>
        {steps.map((step, i) => (
          <>
            <div key={step.label} style={{
              fontFamily:  'var(--font-mono)', fontSize: '8px', padding: '4px 9px', borderRadius: 'var(--radius-sm)',
